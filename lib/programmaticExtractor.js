...
//
// Quality engine pipeline (post-extraction):
//   raw tokens → tokenDeriver (fill gaps) → tokenValidator (guardrails)
// ═══════════════════════════════════════════════════════════

import { deriveTokens } from "./tokenDeriver.js";
import { validateTokens } from "./tokenValidator.js";
...
function parseCSS(cssText) {
  ...
  // Captures :root { ... }, :root[data-theme="dark"] { ... }, etc.
  ...
  while ((rootMatch = rootRegex.exec(cssText)) !== null) {
    ...
    while ((vm = varRegex.exec(rootBlock)) !== null) {
      ...
      // Only set if not already defined (first :root block wins for each var)
      if (!cssVars.has(name)) {
  ...
  // Extract gradients (handle nested parentheses like color stops with rgb())
...
function buildTokenSchema(extracted) {
  ...
  // ── Quality engine pipeline ──────────────────────────
  // Layer 1: Derive missing variants (light bgs, borders, hover, focus, badges, glass)
  // Layer 2: Validate + auto-correct (contrast, hierarchy, scale, clashes)
  const derived = deriveTokens(rawTokens);
  const validated = validateTokens(derived);
...
const NAMED_COLORS = {
  ...
  grey: "#808080", silver: "#C0C0C0", navy: "#000080", teal: "#008080", cyan: "#00FFFF",
  magenta: "#FF00FF", lime: "#00FF00", olive: "#808000", maroon: "#800000", aqua: "#00FFFF",
  coral: "#FF7F50", crimson: "#DC143C", darkblue: "#00008B", darkgreen: "#006400",
  darkgray: "#A9A9A9", darkgrey: "#A9A9A9", darkred: "#8B0000", deeppink: "#FF1493",
  dodgerblue: "#1E90FF", gold: "#FFD700", indigo: "#4B0082", ivory: "#FFFFF0",
  khaki: "#F0E68C", lavender: "#E6E6FA", lightblue: "#ADD8E6", lightgray: "#D3D3D3",
  lightgrey: "#D3D3D3", lightgreen: "#90EE90", lightyellow: "#FFFFE0", limegreen: "#32CD32",
  mediumpurple: "#9370DB", midnightblue: "#191970", mintcream: "#F5FFFA",
  orangered: "#FF4500", orchid: "#DA70D6", plum: "#DDA0DD", royalblue: "#4169E1",
  salmon: "#FA8072", seagreen: "#2E8B57", sienna: "#A0522D", skyblue: "#87CEEB",
  slategray: "#708090", slategrey: "#708090", steelblue: "#4682B4", tomato: "#FF6347",
...
function toHex(val) {
  ...
  // Skip var() references, color-mix(), and other CSS functions we can't resolve
  if (val.startsWith("var(") || val.startsWith("color-mix(")) return null;
