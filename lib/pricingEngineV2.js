...
/**
 * Implementation Spend Multiplier.
 *
 * Scales implementation cost above the Large Enterprise threshold based on
 * total spend under management. Large enterprise deployments above $750M
 * need more rollout effort (more entities, more users, more change
 * management, longer phased rollouts) — the flat tier multiplier doesn't
 * capture that.
 *
 * Formula: clamp(minMult, maxMult, (spend / refSpend)^exponent)
 *
 * Defaults: refSpend=$750M, exponent=0.5 (square root — diminishing
 * returns), minMult=1.0, maxMult=3.0. Below refSpend, returns 1.0
 * (no scaling — tier multipliers handle the low end).
 */
export function calcImplSpendMultiplier(spendM, cfg) {
  const scaling = cfg.implSpendScaling;
  if (!scaling || !scaling.enabled) return 1.0;
  const { refSpend = 750, exponent = 0.5, minMult = 1.0, maxMult = 3.0 } = scaling;
  if (spendM <= refSpend) return minMult;
  const raw = Math.pow(spendM / refSpend, exponent);
...
export function calculatePricingV2(inputs, userConfig) {
  ...
  if (!selectedIntegrations) assumptions.push("Standard integrations assumed (ERP + SSO)");
  ...
  // ─── 10. Deal Parameters (hold for now, discount applied after services) ─
  ...
  // ─── 11. Implementation ──────────────────────────────────────────────
  // Tier-multiplied baseline first, then spend-based multiplier for deals
  // above the Large Enterprise threshold (default $750M). Square-root
  // scaling reflects diminishing-returns reality — a $3B rollout is ~2x
  // the effort of a $750M rollout, not 4x.
  const implCostTierBase = calcImplementation(tier, implToggles, cfg);
  const implSpendMultiplier = calcImplSpendMultiplier(annualSpendM, cfg);
  const implCost = Math.round(implCostTierBase * implSpendMultiplier);
  const addOnCost = calcAddOns(addOns, cfg);
  const totalImplGross = implCost + addOnCost;
  if (implSpendMultiplier > 1.001) {
    const pctDelta = Math.round((implSpendMultiplier - 1) * 100);
    confirmed.push(`Impl spend scaling: ${implSpendMultiplier.toFixed(2)}x (+${pctDelta}% on tier baseline, $${annualSpendM}M spend)`);
  }

  // ─── 12. Gain Share ──────────────────────────────────────────────────
  const gs = { ...cfg.gainShare, ...gainShareOverride };
  let gainShareFeeGross = 0;
  ...
  // ─── 13. Manual Discount — Applied to OVERALL Y1 Total ────────────────
  // The manual discount slider now reduces the ENTIRE Y1 commitment
  // (subscription + services + gain share), not just the subscription.
  // Distributed proportionally across components for clean display and
  // to make the math consistent in the TCO breakdown.
  const discountFactor = 1 - dp.discount / 100;
  const y1SubDiscounted = Math.round(y1Subscription * discountFactor);
  const totalImpl = Math.round(totalImplGross * discountFactor);
  const gainShareFee = Math.round(gainShareFeeGross * discountFactor);
  const manualDiscountAmount = (y1Subscription + totalImplGross + gainShareFeeGross)
    - (y1SubDiscounted + totalImpl + gainShareFee);

  // ─── 14. Y1 Total ────────────────────────────────────────────────────
  const y1GrossTotal = y1Subscription + totalImplGross + gainShareFeeGross;
  const y1Total = y1SubDiscounted + totalImpl + gainShareFee;

  // ─── 15. TCO ─────────────────────────────────────────────────────────
  // Year 1 uses the post-discount values. Year 2+ subscription escalates
  // from year 1 (carrying the discount forward as the permanent rate),
  // matching standard enterprise SaaS contract behavior. Services are
  // Year 1 only (one-time). Gain share escalates the same as subscription.
  const years = [];
  ...
  return {
    ...
    // Subscription
    y1Subscription,           // gross (pre-manual-discount)
    y1SubDiscounted,          // post-manual-discount

    // Implementation
    implCostTierBase,         // after tier multiplier, before spend scaling
    implSpendMultiplier,      // spend-based scaling factor (1.0 below $750M)
    implCost,                 // final impl cost (tier × spend), pre-discount base
    addOnCost,
    totalImplGross,           // totalImpl before manual discount
    totalImpl,                // post-manual-discount services
    implToggles,

    // Gain Share
    gainShare: gs,
    gainShareFeeGross,        // pre-manual-discount
    gainShareFee,             // post-manual-discount
    gainShareNote,

    // Deal & Totals
    dealParams: dp,
    manualDiscountAmount,     // total $ of manual discount across all components
    y1GrossTotal,             // sub + services + gain share, pre-manual-discount
    y1Total,                  // post-manual-discount Y1 commitment
