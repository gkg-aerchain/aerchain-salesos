...
export function calculatePricingV2(inputs, userConfig) {
  ...
  // Dampened, clamped multiplier that scales workflow fees up/down based on
  // actual vs. expected monthly volume. Mirrors the user multiplier pattern
  // (^0.3 dampening, ±15% clamp) so volume affects pricing without
  // dominating it. Only applies when actualTxnVolume is explicitly provided.
  ...
  if (actualTxnVolume != null && actualTxnVolume > 0 && expectedTxnMonth > 0) {
    ...
    const unclamped = Math.pow(volumeRatioRaw, 0.3);
    volumeMultiplier = Math.max(0.85, Math.min(1.15, unclamped));
  ...
  // ─── 5. Platform / Workflow Split (User Mult on Platform, Volume Mult on Workflow) ──
  ...
  const workflowBudgetBase = Math.round(baselineTotal * workflowPct);
