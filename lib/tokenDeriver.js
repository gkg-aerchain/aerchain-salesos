...
export function deriveTokens(tokens) {
  ...
  const gradients = Array.isArray(tokens.gradients) ? [...tokens.gradients] : [];
...
function enrichButtonVariants(components, colors) {
  ...
  const existing = components.button.variants;
...
function deriveGradient(gradients, colors) {
  // Skip if a primary gradient already exists
  if (gradients.some(g => g.name === "Primary")) return;
