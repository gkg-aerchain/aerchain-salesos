import { Upload, Shield, Brain, Clock, CheckCircle, XCircle, AlertTriangle, FileText, Activity } from "lucide-react";
...
const STEPS = [
  ...
  { key: "processing", label: "Analyzing data", icon: Clock },
  { key: "done", label: "Processing complete", icon: CheckCircle },
];

export function StatusPanel({ status, error, processingLabel }) {
  if (!status || status === "idle") return null;

  const steps = processingLabel
    ? STEPS.map(s => s.key === "processing" ? { ...s, label: processingLabel } : s)
    : STEPS;
    ...
    <Card style={{ marginTop: 12 }}>
      ...
      {status === "done" && (
        <div style={{
          ...
          <span style={{ fontSize: 11, color: T.success, fontWeight: 500 }}>Processing complete — results shown below</span>
        </div>
      )}
    </Card>
  );
...
// ── File validation summary bar ─────────────────────────────
export function FileValidationBar({ validationResults }) {
  if (!validationResults || validationResults.results.length === 0) return null;

  const { hasErrors, hasWarnings, results } = validationResults;
